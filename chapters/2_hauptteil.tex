...
\cite{Daradkeh.2023} beschreibt zwei grundlegende Ansätze für die Verwendung von Data-Storytelling: Die manuelle Erstellung von Datengeschichten und die Entwicklung von Softwaretools für die automatisierte Generierung von Datengeschichten.
...
\begin{enumerate}
    ...
    % TODO: Cognitive Load oder Mental Load? weiter oben hieß es mal anders
    % Ordnung schaffen irgendwie hervorheben, zb in Anführungszeichen
    ...
    Um diese Belastung zu reduzieren, kann beispielsweise White Space genutzt werden, indem Lücken zwischen den visuellen Elementen des Programms eingeführt werden. % des Programms oder eines Graphen?
    ...
    % da fehlt vielleicht noch, dass ähnliche Datenpunkten auch ähnlich angeordnet und dargestellt werden etc. -> bisschen mehr die Abbildung beschreiben
    ...
    % den letzten Satz kann man hier auch weglassen
    
    \item Geschichte erzählen\hfill
    
    Im finalen Schritt des Data-Storytellings wird die Geschichte bzw. der Graph logisch aufgebaut. Hierfür müssen ein sinnvoller Anfang und ein Ende festgelegt werden. Zudem soll der Graph ein Fazit und einen Call to Action, also einen Handlungsaufruf, enthalten.
    % TODO: da wäre ein Beispiel für einen logisch aufgebauten und einen nicht logisch aufgebauten Graphen gut
...
% in subsections aufteilen: erst allgemein + welche Oberkategorien, dann kurze subsection zu jeder Oberkategorie

% hier fehlt ein allgemeiner Satz, was für Daten überhaupt dargestellt werden sollen
% den ganzen Absatz umschreiben
...
\begin{enumerate}
    ...
    \item Herausfinden, welche Daten relevant für die Darstellung in den Graphen sind. % Information hat keinen Plural
    \item Definieren, welche Trends in den Zeitreihendaten auftreten und wie diese erkannt werden können.
\end{enumerate}

% in einem Nebensatz beschreiben, was ein qlf ist
...
% die Referenzen vielleicht noch jeweils in Klammern
...
% was bedeuten die 45 Jahre? alle zusammen?
% als vereinfachte T. -> in vereinfachter Form?

%line space in nächster Zeile damit die quelle nicht über den fliestext hinausgeht
Um die Interviews wissenschaftlich zu analysieren, wurde MAXQDA\\ \cite{MAXQDA.2023} verwendet und nach den Methoden von \cite{Mayring.2022}, \cite{Kuckartz.2022} und \cite{AndreMorgensternEinenkel.2023} die inhaltlich strukturierende qualitative Inhaltsanalyse erstellt.

% Was sagt der Absatz?
%Anhand dieser Analyse konnten mehrere Kategorien festgelegt werden, die für das Erkennen der Trends in den Daten von Bedeutung sind. Insbesondere die Kategorien in der Überkategorie Trends sind von Wichtigkeit, da in dieser Kategorie genau die Aussagen zu den Trends erkennbar sind.
...
% die Kategorien sind nicht sortiert -> "mit Häufigkeit der Erwähnung"
\begin{figure}[h!]
    \centering
...
% was sind Extremwerte? -> absolutes min+max des betrachteten Signals
% Ausreißer im Bezug auf was? -> Vergleich mit ähnlichen Messungen
...
% TODO: was heißt "dynamisch" an der Stelle? ohne Nutzereingaben?
...
% in C -> "in Appendix C" oder so (könnte im Satzkontext sonst heißen, dass das mit der Programmiersprache C schon irgendwo implementiert war)
% Absatz umschreiben, z.B. immer die gleiche Distanz zueinander -> äquidistant
...
% unterschiedlich formatierte Verweise auf den Source Code -> z.B. (Implementierung s. Appendix C, Z. 7-9)
% Parameter mit angeben, sonst muss man in den Appendix blättern, um die Beschreibung nachzuvollziehen
...
% das heißt nicht height, sondern range
...
% der Name passt hier nicht zu dem, was berechnet wird
...
% passt die formel hier? xi in 1,2,..n
...
% TODO: erklärung vereinfachen? ist ja im prinzip nur gleitender durchschnitt -> streng monotone Bereiche suchen, deren range einen schwellwert überschreitet
...
% sowas wie dass slice() verwendet wird, kann man streichen, das ist selbstverständlich. lenkt nur davon ab was tatsächlich passiert
...
% erst kurz beschreiben, was die Datenbasis überhaupt beinhaltet, z.b. weiß man beim lesen noch nicht, dass es eine fahrzeugkarte gibt und wie man fahrzeuge auswählt
...
% \begin{description}
%   \item[Level Crossing (dt. Klassengrenzenüberschreitungszählung)] \hfill \\ 
...
%   \item[Range Pair (dt. Bereichspaarzählung)] \hfill \\ 
...
%   \item[Time at Level (dt. Verweildauerzählung)] \hfill \\ 
